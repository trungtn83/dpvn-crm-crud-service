...
//      List<CustomerType> customerTypes = customerTypeService.findAllActive();
...
//          dbActiveSaleCustomer.getId(), FastMap.create().add("active", false).add("deleted",
// true));
//    }
//
//    return null;
...
//            .findById(saleId)
//            .orElseThrow(
//                () -> new BadRequestException(String.format("Sale with id %s not found",
// saleId)));
//    Customer customer =
//        findById(customerId)
//            .orElseThrow(
...
//        saleId, filterText, typeIds, locationCodes, sourceIds, pageable);
//  }
//
...
//          .getReferences()
//          .removeIf(r ->
// Customers.References.LAST_SUCCESSFUL_TRANSACTION_DATE.equals(r.getCode()));
...
//    injectCustomerType(entity);
//    entity.getReferences().forEach(ref -> ref.setCustomer(entity));
//    Customer dbEntity = super.create(entity);
//
//    updateAddresses(dbEntity.getId(), entity.getAddresses());
//
//    return dbEntity;
//  }
//
//  private void updateAddresses(Long ownerId, List<Address> addresses) {
//    addresses.forEach(
//        ca -> {
//          ca.setOwnerType("CUSTOMER");
//          ca.setOwnerId(ownerId);
//        });
//    addressRepository.deleteByOwnerTypeAndOwnerId("CUSTOMER", ownerId);
...
//
//    List<Address> addresses = data.getListClass("addresses", Address.class);
